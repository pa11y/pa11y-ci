...
function pa11yCi(urls, options) {
	return new Promise(async resolve => {
		...
		// Resolve reporters
		const reporters = [].concat(options.reporters).map(reporter => {
		...
		await cycleReporters(reporters, 'start', urls);
		...
		function testRunComplete() {
			...
			cycleReporters(reporters, 'finish', urls).then(() => resolve(report));
		}
