...
function pa11yCi(urls, options) {
	return new Promise(resolve => {
		// Initialise a pool to store reusable browsers that we
		// will use to run our tests
		const browserPool = [];
		...
		async function testRunner(config) {
			const browserConfig = config.chromeLaunchConfig;
			let url;
			...
			// Get a free browser if there's one available and if
			// it was opened with suitable configuration
			const availableBrowser = browserPool.pop();
			if (availableBrowser && isEqual(browserConfig, availableBrowser.browserConfig)) {
				config.browser = availableBrowser.browser;
			} else {
				config.browser = await puppeteer.launch(browserConfig);
			}
			...
			// Release the browser back into the available pool
			browserPool.push({
				browserConfig,
		...
		function testRunComplete() {
			...
			for (const runner of browserPool) {
