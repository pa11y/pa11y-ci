...
function pa11yCi(urls, options) {
	return new Promise(resolve => {
		...
		function testRunner(config, done) {
			...
			pa11yTest.run(config, (error, results, documentTitle) => {
				if (error) {
				...
				if (options.invalidDocTitle && documentTitle === options.invalidDocTitle) {
					message += chalk.red(`Test conducted on invalid document: ${documentTitle}`);
					log.error(message);
					report.errors += 1;
