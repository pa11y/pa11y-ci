...
function pa11yCi(urls, options) {
	return new Promise(resolve => {
		...
		function testRunner(config, done) {
			...
			pa11yTest.run(config, (error, result) => {
				if (error) {
				...
				if (options.invalidDocTitle && result.documentTitle === options.invalidDocTitle) {
					message += chalk.red(`Test conducted on invalid document: ${result.documentTitle}`);
					log.error(message);
					report.errors += 1;
