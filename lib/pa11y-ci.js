...
// Puppeteer's close() can hang if called while a navigation/evaluation
// against the target is still in process (e.g. after a pa11y timeout).
// Race it against a timer so a stuck close can never hang the whole run.
function closeWithTimeout(closeable, ms = 10000) {
	return Promise.race([
		closeable.close(),
	...
	return new Promise(async (resolve, reject) => {
		...
		try {
			...
			async function testRunner(config) {
				...
				try {
					...
					if (config.useIncognitoBrowserContext) {
						await closeWithTimeout(config.browser);
					}
				}
			}
			...
			async function testRunComplete() {
				try {
					await closeWithTimeout(testBrowser);
			...
			// Ensure the browser is not left dangling if setup fails
			if (testBrowser) {
				await closeWithTimeout(testBrowser);
			}
			reject(error);
